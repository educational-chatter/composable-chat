...
fun ComposableChatAppBar(
    ...
    val backgroundColors = TopAppBarDefaults.centerAlignedTopAppBarColors(containerColor = MaterialTheme.colors.surface)
    ...
    Box(modifier = Modifier.padding(bottom = 4.dp).background(backgroundColor)) {
...
fun ComposableChatAppBarPreview() {
    ComposableChatTheme {
        ComposableChatAppBar(title = { Text("Поддержка") })
    }
}
